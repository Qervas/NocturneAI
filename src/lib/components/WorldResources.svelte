<script lang="ts">
  ...
  import type { WorldResources } from '../types/Perks';

  // Component state
  let showWorldResources = false;
  let worldResources: WorldResources;
  let simulationStats = { tickCount: 0, totalRunTime: 0, agentActionCount: 0 };
  ...
  // Subscribe to stores
  perkManager.worldResources.subscribe(value => {
    worldResources = value;
  });
  ...
  function getResourceColor(value: number, threshold: { good: number, warning: number }): string {
  ...
  onMount(() => {
    ...
    const interval = setInterval(() => {
      if (worldResources) {
        worldResources.totalAgents = agentList.length;
        worldResources.activeAgents = activeAgents;
        worldResources.uptime = simulationStats.totalRunTime;
      }
    }, 2000);
...
{#if showWorldResources}
  <div class="world-resources-panel">
    <div class="resources-header">
      ...
      <button class="toggle-btn" on:click={toggleWorldResources}>👁️</button>
    </div>
      ...
      <div class="resource-section">
        ...
        <div class="resource-stats">
          <div class="stat-row">
            ...
            <span class="stat-value" style="color: {getResourceColor(activeAgents, {good: 2, warning: 1})}">
              {activeAgents}/{worldResources?.totalAgents || 0}
            </span>
          </div>
          <div class="stat-row">
            <span class="stat-label">Online Rate:</span>
            <span class="stat-value">
              {worldResources?.totalAgents ? Math.round((activeAgents / worldResources.totalAgents) * 100) : 0}%
            </span>
          </div>
        </div>
      </div>
        ...
        <div class="resource-stats">
          <div class="stat-row">
            ...
            <span class="stat-value experience-value">
              {formatNumber(worldResources?.totalExperience || 0)}
            </span>
          </div>
          <div class="stat-row">
            <span class="stat-label">Tasks Done:</span>
            <span class="stat-value">
              {formatNumber(worldResources?.completedTasks || 0)}
            </span>
          </div>
        </div>
      </div>
      ...
      <div class="resource-section">
        ...
        <div class="resource-stats">
          <div class="stat-row">
            ...
            <span class="stat-value" style="color: {getResourceColor(worldResources?.successRate || 0, {good: 80, warning: 60})}">
              {Math.round(worldResources?.successRate || 0)}%
            </span>
          </div>
          <div class="stat-row">
      ...
      <div class="resource-section">
        ...
        <div class="resource-stats">
          <div class="stat-row">
            ...
            <span class="stat-value">
              {formatTime(worldResources?.uptime || 0)}
            </span>
          </div>
          <div class="stat-row">
      ...
      <div class="resource-section">
        ...
        <div class="resource-stats">
          <div class="stat-row">
            ...
            <span class="stat-value">
              {formatNumber(1000 + (worldResources?.completedTasks || 0) * 50)}
            </span>
          </div>
          <div class="stat-row">
    ...
    <div class="status-indicators">
      ...
      <div class="status-item">
        <div class="status-dot" class:active={(worldResources?.successRate || 0) > 70}></div>
        <span>Performance</span>
      </div>
      <div class="status-item">
        <div class="status-dot" class:active={communicationStats.totalMessages > 0}></div>
...
<style>
  ...
  .world-resources-panel {
    ...
    top: 140px;
    left: 20px;
    background: linear-gradient(135deg, rgba(0, 0, 0, 0.92) 0%, rgba(0, 20, 40, 0.92) 100%);
